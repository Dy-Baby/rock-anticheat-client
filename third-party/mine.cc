...
#ifndef MINE_VERSION
#define MINE_VERSION XorStr("1.1.4")
#endif
...
const std::string Base16::kValidChars = XorStr("0123456789ABCDEF");
...
void Base16::decode(char a, char b, std::ostringstream& ss)
{
    try {
        ...
        throw std::invalid_argument(XorStr("Invalid base-16 encoding: ") + std::string(e.what()));
    }
}
...
void AES::setKey(const ByteArray& key)
{
    if (key.size() != 16 && key.size() != 24 && key.size() != 32) {
        throw std::invalid_argument(XorStr("Invalid key size. AES can operate on 128-bit, 192-bit and 256-bit keys"));
    }
    m_key = key;
...
void AES::printBytes(const ByteArray& b)
{
    for (std::size_t i = 1; i <= b.size(); ++i) {
        std::cout << XorStr("0x") << (b[i - 1] < 10 ? XorStr("0") : "") << Base16::encode(b[i - 1]) << "  ";
        if (i % 4 == 0) {
            std::cout << std::endl;
        }
    }
...
void AES::printState(const State* state)
{
    for (std::size_t i = 0; i < kNb; ++i) {
        for (std::size_t j = 0; j < kNb; ++j) {
            byte b = (*state)[j][i];
            std::cout << XorStr("0x") << (b < 10 ? XorStr("0") : "") << Base16::encode(b) << "  ";
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
...
AES::KeySchedule AES::keyExpansion(const Key* key)
{
    ...
    if (keySize != 16 && keySize != 24 && keySize != 32) {
        throw std::invalid_argument(XorStr("Invalid AES key size"));
    }
...
ByteArray AES::encryptSingleBlock(const ByteArray::const_iterator& range, const Key* key, KeySchedule* keySchedule)
{
    ...
    if (key == nullptr || keySchedule == nullptr) {
        throw std::invalid_argument(XorStr("AES raw encryption requires key"));
    }
...
ByteArray AES::decryptSingleBlock(const ByteArray::const_iterator& range, const Key* key, KeySchedule* keySchedule)
{
    ...
    if (key == nullptr || keySchedule == nullptr) {
        throw std::invalid_argument(XorStr("AES raw decryption requires key"));
    }
...
std::size_t AES::getPaddingIndex(const ByteArray& byteArr)
{
    ...
    if (c > 0 && c <= kBlockSize) {
        ...
        if (validPadding) {
            ...
            throw std::runtime_error(XorStr("Incorrect padding"));
        }
    }
    return kBlockSize;
}
...
ByteArray AES::encrypt(const ByteArray& input, const Key* key, ByteArray& iv, bool pkcs5Padding)
{
    ...
    if (!iv.empty() && iv.size() != 16) {
        throw std::invalid_argument(XorStr("Invalid IV, it should be same as block size"));
    }
    else if (iv.empty()) {
        // generate IV
...
ByteArray AES::decrypt(const ByteArray& input, const Key* key, ByteArray& iv)
{
    ...
    if (inputSize % kBlockSize != 0) {
        throw std::invalid_argument(XorStr("Ciphertext length is not a multiple of block size"));
    }
...
std::string AES::generateRandomKey(const std::size_t len)
{
    if (len != 128 && len != 192 && len != 256) {
        throw std::invalid_argument(XorStr("Please choose valid key length of 128, 192 or 256 bits"));
    }
...
{
    if (m_key.empty()) {
        throw std::runtime_error(XorStr("Key not set"));
    }
...
{
    gzFile out = gzopen(gzFilename.c_str(), XorStr("wb"));
    if (!out) {
        throw std::invalid_argument(XorStr("Unable to open file [") + gzFilename + XorStr("] for writing.") + std::strerror(errno));
    }
    ...
    while ((nRead = std::fread(buff, sizeof(char), kBufferSize, in)) > 0) {
        ...
        if (bytes_written == 0) {
            int err_no = 0;
            throw std::runtime_error(XorStr("Error during compression: ") + std::string(gzerror(out, &err_no)));
            gzclose(out);
            return false;
        }
    }
...
std::string ZLib::compressString(const std::string& str)
{
    ...
    if (deflateInit(&zs, compressionlevel) != Z_OK) {
        throw std::runtime_error(XorStr("Unable to initialize zlib deflate"));
    }
    ...
    if (ret != Z_STREAM_END) {
        throw std::runtime_error(XorStr("Exception during zlib decompression: (") + std::to_string(ret) + XorStr("): ") + std::string((zs.msg != NULL ? zs.msg : XorStr("no msg"))));
    }
...
std::string ZLib::decompressString(const std::string& str)
{
    ...
    if (inflateInit(&zs) != Z_OK) {
        throw std::runtime_error(XorStr("Unable to initialize zlib inflate"));
    }
