...
public:
    ...
    {
        if (enc.size() % 2 != 0) {
            throw std::invalid_argument(XorStr("Invalid base-16 encoding"));
        }
    ...
    static T decodeInt(const std::string& e)
    {
        ...
        for (auto it = e.begin(); it != e.end() && result >= 0; ++it) {
            try {
                ...
                throw std::runtime_error(XorStr("Invalid base-16 encoding"));
            }
        }
        return result;
    }
    ...
    static std::string decode(const Iter& begin, const Iter& end)
    {
        ...
        for (auto it = begin; it < end; it += 4) {
            try {
                ...
                int b0 = findPosOf(*it);
                if (b0 == kPadding) {
                    throw std::invalid_argument(XorStr("No data available"));
                }
                if (b0 == -1) {
                    throw std::invalid_argument(XorStr("Invalid base64 encoding"));
                }
                ...
                throw std::invalid_argument(std::string(XorStr("Invalid base64 encoding: ") + std::string(e.what())));
            }
        }
result:
        return ss.str();
...
public:
    ...
    virtual BigIntegerT modInverse(BigIntegerT a, BigIntegerT m) const
    {
        ...
        if (gcdResult != 1) {
            throw std::invalid_argument(XorStr("Inverse does not exist"));
        }
        /*std::cout << x << std::endl;
    ...
    {
        std::string readableMsg = XorStr("0x") + hex;
...
public:
    ...
    void init(const BigIntegerT& n)
    {
        ...
        if (m_k < 11) {
            throw std::invalid_argument(XorStr("Invalid prime. Length error."));
        }
    }
...
public:
    ...
    {
        if (p == q || p == 0 || q == 0) {
            throw std::invalid_argument(XorStr("p and q must be prime numbers unique to each other"));
        }
        m_p = p;
        m_q = q;
        m_e = e;
        ...
        if (BaseKey::m_helper.gcd(m_e, phi) != 1) {
            throw std::invalid_argument(XorStr("Invalid exponent, it must not share factor with phi"));
        }
        BaseKey::m_n = m_p * m_q;
    ...
    {
        ss << XorStr("modulus: ") << k.m_n << XorStr("\npublicExponent: ") << k.m_e << XorStr("\nprivateExponent: ") << k.m_d
           << XorStr("\nprime1: ") << k.m_p << XorStr("\nprime2: ") << k.m_q << XorStr("\nexponent1: ") << k.m_dp << XorStr("\nexponent2: ")
    ...
    {
        std::stringstream ss;
        ss << XorStr("asn1=SEQUENCE:rsa_key\n\n");
        ss << XorStr("[rsa_key]\n");
        ss << XorStr("version=INTEGER:0\n");
        ss << XorStr("modulus=INTEGER:") << BaseKey::m_helper.bigIntegerToString(BaseKey::m_n) << "\n";
        ss << XorStr("pubExp=INTEGER:") << m_e << "\n";
        ss << XorStr("privExp=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_d) << "\n";
        ss << XorStr("p=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_p) << "\n";
        ss << XorStr("q=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_q) << "\n";
        ss << XorStr("e1=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_dp) << "\n";
        ss << XorStr("e2=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_dq) << "\n";
        ss << XorStr("coeff=INTEGER:") << BaseKey::m_helper.bigIntegerToString(m_coeff);
        return ss.str();
    }
protected:
    BigIntegerT m_p;
...
public:
    ...
    TResult decrypt(const PrivateKey* privateKey, const std::string& c)
    {
        ...
        if (msg >= m_helper.power(MathHelper<BigIntegerT>::kBigIntegerT256, BigIntegerT(xlen))) {
            throw std::runtime_error(XorStr("Integer too large"));
        }
...
private:
    ...
    BigIntegerT addPadding(const T& s, std::size_t n) {
        if (n < s.size() + 11) {
            throw std::runtime_error(XorStr("Message too long"));
        }
    ...
    T removePadding(const RawString& ba)
    {
        ...
        if (baLen <= 2 || ba[0] != 0 || ba[1] != 2) {
            throw std::runtime_error(XorStr("Incorrect padding PKCS#1"));
        }
    ...
    BigIntegerT createVerificationPrimitive(const PublicKey* publicKey, const BigIntegerT& signature)
    {
        if (signature < 0 || signature > publicKey->n() - 1) {
            throw std::runtime_error(XorStr("signature representative out of range"));
        }
    ...
    BigIntegerT createSignaturePrimitive(const PrivateKey* privateKey, const BigIntegerT& msg)
    {
        if (msg < 0 || msg > privateKey->n() - 1) {
            throw std::runtime_error(XorStr("message representative out of range"));
        }
